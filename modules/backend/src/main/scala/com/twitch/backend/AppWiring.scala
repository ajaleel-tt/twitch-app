...
object AppWiring:
  ...
  def build(
    ...
    for
      ...
      authRoutes = new routes.AuthRoutes(config.clientId, config.redirectUri, twitchApi, pendingOAuthStates, userRepo, sessionRepo, emailService)
      apiRoutes = new routes.ApiRoutes(config.clientId, sessionManager, twitchApi, followRepo, tagFilterRepo, ignoredStreamerRepo, sessionRepo, pushRepo, topGamesRepo, notificationQueues, settings)
      ...
      httpApp = Router(
        ...
        "/" -> HttpRoutes.of[IO] {
          case req @ GET -> Root =>
            StaticFile.fromPath(fs2.io.file.Path(s"${config.staticDir}/index.html"), Some(req)).getOrElseF(NotFound())
        },
        "/" -> frontendService
      ).orNotFound
      corsApp = CORS.policy.withAllowOriginAll(httpApp)
      poller <- StreamPoller.make(config.clientId, config.clientSecret, client, followRepo, tagFilterRepo, ignoredStreamerRepo, pushRepo, notificationQueues, settings, pushService)
      topGamesPoller <- TopGamesPoller.make(config.clientId, config.clientSecret, client, topGamesRepo, settings)
