...
object View:
  def view(model: Model): Html[Msg] =
    div(style("width", "100%"), style("display", "flex"), style("flex-direction", "column"), style("align-items", "center"))(
      ...
      model.user.map { u =>
        val paginatedResults = model.searchResults.slice(model.currentPage * model.pageSize, (model.currentPage + 1) * model.pageSize)
        val totalLocalPages = (model.searchResults.size + model.pageSize - 1) / model.pageSize
        ...
        div(style("width", "100%"), style("display", "flex"), style("flex-direction", "column"), style("align-items", "center"))(
          ...
          if model.searchResults.nonEmpty then
            div(style("display", "flex"), style("flex-direction", "column"), style("align-items", "center"), style("margin-top", "20px"))(
              div(style("display", "flex"), style("gap", "10px"), style("align-items", "center"))(
                button(
                  onClick(Msg.SetPage(model.currentPage - 1)),
                  disabled(model.currentPage == 0),
                  style("padding", "5px 15px")
                )("Previous"),
                span(s"Page ${model.currentPage + 1} of $totalLocalPages"),
                button(
                  onClick(Msg.SetPage(model.currentPage + 1)),
                  disabled(model.currentPage >= totalLocalPages - 1),
                  style("padding", "5px 15px")
                )("Next")
              ),
              if (model.paginationCursor.isDefined)
                button(
                  onClick(Msg.FetchMore), 
