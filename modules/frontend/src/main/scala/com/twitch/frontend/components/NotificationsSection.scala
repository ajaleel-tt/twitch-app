          ...
          if relevant.isEmpty then
            List(
              div(
                ...
                p(
                  cls := "text-gray-500",
                  "No streams detected live yet. Notifications will appear here.",
                ),
              ),
            )
          else relevant.map(n => notificationCard(state, n))
        },
      ),
    )

  private def notificationCard(
    ...
    div(
      ...
      div(
        ...
        onClick --> {
          _.foreach(_ =>
            IO {
              val _ = org.scalajs.dom.window.open(s"https://twitch.tv/${n.streamerLogin}", "_blank")
            },
          )
        },
