...
object SearchSection:
      ...
      div(
        ...
        div(
          ...
          button(
            ...
            "Cancel",
            onClick --> { _.foreach(_ => state.update(_.copy(pendingPopularFollow = None))) },
          ),
          button(
                      ...
                      state.update(_.copy(pendingPopularFollow = None)) *>
                        (ApiClient
                          ...
                          .flatMap(_ =>
                            ApiClient
                              .fetchFollowed
                              .flatMap(cats => state.update(_.copy(followedCategories = cats))),
                          ))
                          .start
                          .void
                    case None => IO.unit
      ...
      if m.search.query.trim.isEmpty then IO.unit
        ...
        state.update(m =>
          m.copy(
            ...
            search = m.search.copy(results = Vector.empty, paginationCursor = None, currentPage = 0),
          ),
        ) *>
          ...
          onInput --> {
            _.foreach(_ =>
              self
                .value
                .get
                .flatMap(q => state.update(m => m.copy(search = m.search.copy(query = q)))),
            )
          },
  ...
  def paginationView(state: SignallingRef[IO, Model]): Resource[IO, HtmlDivElement[IO]] =
    div(
      ...
      div(
        ...
        button(
          ...
          onClick --> {
            _.foreach(_ =>
              state.update(m =>
                m.copy(search = m.search.copy(currentPage = m.search.currentPage - 1)),
              ),
            )
          },
        ),
      ...
      button(
        cls <-- state.map { m =>
          if m.search.paginationCursor.isDefined then
            List(
              "bg-twitch-purple",
                      ...
                      state.update(m =>
                        m.copy(
                          search = m
                            .search
                            .copy(
                              results = m.search.results ++ res.data,
                              paginationCursor = res.pagination.flatMap(_.cursor),
                            ),
                          status = None,
                        ),
      ...
      cls <-- state.map { m =>
        ...
        if isSelected then
          base ++ List(
            ...
            "ring-twitch-purple",
            "shadow-lg",
            "shadow-twitch-purple/20",
          )
        else
          base ++ List(
            "border-gray-800",
            "hover:border-twitch-purple",
            "hover:shadow-lg",
            "hover:shadow-twitch-purple/10",
          )
      },
      div(
        ...
        onClick --> {
          _.foreach(_ =>
            state.update(m =>
              val newSel = if m.search.selectedCategoryIds.contains(cat.id) then
                m.search.selectedCategoryIds - cat.id
              else m.search.selectedCategoryIds + cat.id
